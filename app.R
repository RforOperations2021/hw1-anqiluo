...
library(tools)
library(dplyr)
test = read.csv('/Users/anqiluo/Desktop/Carnegie Mellon/2021 Spring/Rshiny/HW1-anqiluo/2006_-_2011_NYS_Math_Test_Results_by_Grade_-_Citywide_-_by_Race-Ethnicity.csv')
#test = read.csv('2006_-_2011_NYS_Math_Test_Results_by_Grade_-_Citywide_-_by_Race-Ethnicity.csv')

ui <- fluidPage(
  downloadButton('downloadData', class = "butt1"),
...
server <- function(input, output,session) {
  ...
  output$scatter <- renderPlot({
    ggplot(data = test,aes_string(x = 'Grade', y = 'Number.Tested')) +
    labs(title='Scatter plot of Number Tested Grouped by Grade') +
    geom_point() +
    theme(
  ...
  output$hist = renderPlot({
    hist(grade_sample()$Mean.Scale.Score,
      xlab = 'Mean.Scale.Score',main =paste('Histogram of Mean Scale Score for Grade', sapply(list(input$selected_grade), paste, collapse = ", ")))
  })
